...
$testurl = 'http://www.228365365.com/app/member/FT_future/body_var.php?uid=test00&rtype=r&langx=zh-cn&mtype=3&page_no=0&league_id=&hot_game=';
//第一次抓cookie
$ch=curl_init();
...
$test = substr($pagecontent,65);
$arr = explode('parent.GameFT', $test);

$insert = new League();
//先清除資料表
$insert->deleteGameData();
for ($i = 1 ; $i < count($arr) ; $i++)
{
    ...
    for ($j = 1 ; $j <= 3 ; $j++){
        if ($j == 1) {
            ...
            $insert->insertGameData($league, $time, $teamName, $capott1, $overAllHandicap, $overAllSize, $oddOrEven, $capott2, $halfHandicap, $halfSize);
        } elseif ($j == 2) {
            $league = $newarr[$i][2];
            $time = substr($newarr[$i][1], 10, 5);
            $teamName = $newarr[$i][6];
